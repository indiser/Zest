...
def evaluate(board):
    ...
    for square in chess.SQUARES:
        ...
        if piece.piece_type == chess.KNIGHT:
            ...
            if is_endgame:
                ...
                if rank == 3: pawn_val += 50
                if rank == 4: pawn_val += 150
                if rank == 5: pawn_val += 200
                if rank == 6: pawn_val += 500
            value += pawn_val
    ...
    white_safety = king_safety_score(board, chess.WHITE, is_endgame)
    black_safety = king_safety_score(board, chess.BLACK, is_endgame)
